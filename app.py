...
Versi: Stabil untuk Streamlit Cloud (Python 3.10 compatible)
Kelas: Arca, dolmen, menhir, dakon, batu_datar, Kubur_batu, Lesung_batu
"""

# ==============================================
# FIX FOR PROTUBUF AND COMPATIBILITY ISSUES
# ==============================================
import os
os.environ["PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION"] = "python"
os.environ["TF_CPP_MIN_LOG_LEVEL"] = "2"  # Kurangi logging TensorFlow
...
def load_tflite_interpreter(model_path):
    # Langsung pakai tensorflow (lebih stabil)
    try:
    ...
    if not os.path.exists(model_path) or os.path.getsize(model_path) < 10000:
        with st.spinner("🔄 Mendownload model (±96 MB)... Harap tunggu."):
            try:
                ...
                st.info("💡 Pastikan file di Google Drive sudah di-share sebagai 'Anyone with the link'")
                return None, None, None

    interp = load_tflite_interpreter(model_path)
    if interp is None:
        st.error("❌ Tidak dapat memuat model TFLite.")
        return None, None, None
...
if interpreter is None:
    ...
    Pastikan:
    1. FILE_ID sudah benar di `app.py`
